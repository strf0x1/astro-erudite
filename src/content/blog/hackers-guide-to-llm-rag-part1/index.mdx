...
The cybersecurity community has seen many AI hype cycles over the years, often leading to skepticism when new AI technologies emerge. This skepticism is well-earned – we've learned that there's rarely an easy button in security, and simple solutions can create dangerous blind spots.
But this time feels a little different.

Yet with recent developments in AI, particularly in local language models and retrieval systems, offer some genuinely practical tools for researchers. We'll explore how to build something useful: a private, customizable knowledge base that works the way you need it to work.

### Working with Local LLMs

Local Language Models (LLMs) represent an interesting addition to a security researcher's toolkit. While they're not a silver bullet, they can be particularly effective for specific tasks when used thoughtfully.
...
For instance, you could use Python and [ollama](https://ollama.com/) to analyze potential phishing content. Here's a simple example:
...
However, they excel at other tasks like code analysis, pattern identification, and converting unstructured data into structured formats.
