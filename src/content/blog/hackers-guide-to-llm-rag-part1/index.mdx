...
The Cybersecurity community has been bombarded by AI hype trains for years, even before ChatGPT’s rise to fame. There were promises of magical artificial intelligence fairies that solved all our problems.

In practice, many of us learned there was no easy button. It’s a game of cat-and-mouse in a field that is constantly changing, and easy buttons develop complacency, leading to a false sense of security and making our systems more opaque.

For that reason, I’m not all that surprised by the skepticism I hear when talking to people in Cybersecurity about the recent advancements in AI. We’ve been burned by the hype before.

This time, it feels different, but perhaps not in the way that might initially make sense.

## Why The Shift?


Most traditional Cybersecurity AI models were relatively simple. Detection methods relied on simpler algorithms like random forests and anomaly detection. These techniques had some major drawbacks: they were slow, computationally expensive, and required large datasets for baseline establishment.

Random Forests relied on spotting patterns in historical data. This worked well for simple attacks, but hackers constantly evolve their techniques. Think of it like a signature-based antivirus: if a virus changes its code even slightly, the old signature won't recognize it.

Then ChatGPT 3 dropped, and the AI community saw an explosion of growth. Computer hardware caught up, allowing us to soak up the influx of investment pouring into the field. Dozens of groundbreaking research papers seem to be released every week. Now you can run sophisticated language models from a home computer or in the cloud for pennies.
...
Next, you’ll want a relatively recent version of Python. Then, to make things much easier to manage, we’ll be using `venv` and `poetry` (to manage dependencies):
...
Our example code should work fairly well, but we already have some engineering challenges. First, we used an in-memory vector store to start with, which makes it simpler to get started but won’t scale well. We should build a more permanent vector database that will survive reboots and outages.
