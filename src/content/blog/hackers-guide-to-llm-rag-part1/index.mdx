...
Traditional SIEM AI strategies relied on simpler algorithms and methods. These techniques had major drawbacks: they were slow, computationally expensive, and required large datasets for baseline establishment. We're not really going to applying new techniques to SIEM's, rather looking at other interesting applications of machine learning tools.

It seems like every day my feed on X is inundated with new opensource LLM tools, and some of them are really useful.
